...
/**
 * Binds a {@see Signer} and an {@see Encrypter} under a fixed key, so a harness
 * gets the working signer the CSRF guard needs while still booting from the bare, .env-less Environment
 * integration tests deliberately use.
 *
 * The production providers read APP_KEY through Environment::required(), which
 * is the binding this one replaces.
 */
final class FixedSignerServiceProvider extends ServiceProvider
{
    ...
    public function register(ContainerInterface $container): void
    {
        ...
        $container->instance(Encrypter::class, Encrypter::fromHex(self::KEY_HEX));
    }
}
